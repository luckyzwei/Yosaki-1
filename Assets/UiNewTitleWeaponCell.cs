...
public class UiNewTitleWeaponCell : MonoBehaviour
{
    ...
    private Title_WeaponData requireTableData;
    ...
    {
        ServerData.userInfoTable.GetTableData(UserInfoTable.titleWeapon).AsObservable().Subscribe(e =>
        {
            ButtonInteractableCheck();
            //다음단계가 Length 이상이 될시.
            lockMask.SetActive(e + 1 >= TableManager.Instance.titleWeapon.dataArray.Length);
        }).AddTo(this);
    }
    ...
    public void Initialize()
    {
        ...
        if (currentLevel + 1 >= TableManager.Instance.titleWeapon.dataArray.Length)
        {
        ...
        requireTableData = TableManager.Instance.titleWeapon.dataArray[Mathf.Min((int)currentLevel + 1,TableManager.Instance.titleWeapon.dataArray.Length - 1)];
        ...
        currentAbilDescription.SetText($"{CommonString.GetStatusName((StatusType)requireTableData.Abiltype1)} {Utils.ConvertBigNum(PlayerStats.GetTitleWeaponAbilValue((StatusType)requireTableData.Abiltype1)*100)}% 증가");
        ButtonInteractableCheck();
    }
    private void ButtonInteractableCheck()
    {
        rewardButton.interactable =
            ServerData.weaponTable.TableDatas[$"weapon{requireTableData.Condition}"].hasItem.Value > 0;
    }
    ...
    public void OnClickRewardButton()
    {
        ...
        if (ServerData.userInfoTable.GetTableData(UserInfoTable.titleWeapon).Value + 1 >=
            TableManager.Instance.titleWeapon.dataArray.Length)
        {
            PopupManager.Instance.ShowAlarmMessage("다음 업데이트를 기다려주세요!");
            return;
        }
        
        if (ServerData.weaponTable.TableDatas[$"weapon{requireTableData.Condition}"].hasItem.Value<1)
        {
            PopupManager.Instance.ShowAlarmMessage("무기가 없습니다.");
            return;
        }

        ServerData.userInfoTable.GetTableData(UserInfoTable.titleWeapon).Value++;
        
        List<TransactionValue> transactions = new List<TransactionValue>();

        Param userinfoParam = new Param();

        userinfoParam.Add(UserInfoTable.titleWeapon,
            ServerData.userInfoTable.GetTableData(UserInfoTable.titleWeapon).Value);
