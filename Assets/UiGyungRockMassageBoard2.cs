...
public class UiGyungRockMassageBoard2 : MonoBehaviour
{
    ...
    {
        totalAbil.SetText(
            ($"{CommonString.GetStatusName((StatusType.SuperCritical13DamPer))} 총 {PlayerStats.GetGyungRockEffect2(StatusType.SuperCritical13DamPer) * 100f}% 적용됨"));
        ...
        if (currentFloor >= TableManager.Instance.gyungRockTowerTable2.dataArray.Length)
        {
        ...
        dokChimEffect.SetText($"전갈굴(문파) 독침 효과로 중단전베기 피해량 {Utils.ConvertBigNumForRewardCell(Mathf.Round(PlayerStats.GetGuildTowerChimUpgradeValue() * 100f))}% 증폭됨<color=yellow>(+{PlayerStats.GetGuildTowerChimUpgradeValue() * PlayerStats.GetGyungRockEffect2(StatusType.SuperCritical13DamPer) * 100f}%)</color>");
    ...
    public void OnClickEnterButton()
    {
        ...
        if (currentFloor < 0 || currentFloor >= TableManager.Instance.gyungRockTowerTable2.dataArray.Length)
        {
            PopupManager.Instance.ShowAlarmMessage(("최고 단계 입니다!"));
            return;
        }

        PopupManager.Instance.ShowYesNoPopup(CommonString.Notice, "입장 하시겠습니까?", () => { GameManager.Instance.LoadContents(GameManager.ContentsType.GyungRockTower2); }, () => { });
    }
