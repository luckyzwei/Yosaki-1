...
public class UiYoguiSogulRankBoard : MonoBehaviour
{
    ...
    private void Subscribe()
    {
        RankManager.Instance.WhenMyRelicRankLoadComplete.AsObservable().Subscribe(e =>
        {
            if (e != null)
            {
                myRankView.Initialize($"{e.Rank}", e.NickName, $"{Utils.ConvertBigNum(e.Score)}점", e.Rank, e.costumeIdx, e.petIddx, e.weaponIdx, e.magicbookIdx, e.gumgiIdx, e.GuildName,e.maskIdx,e.hornIdx,e.suhoAnimal);
            }
            else
            {
            ...
            if (rows.Count > 0)
            {
                ...
                for (int i = 0; i < rankViewContainer.Count; i++)
                {
                    ...
                    nickName = nickName.Replace(CommonString.IOS_nick, "");
                        ...
                        rankViewContainer[i].Initialize($"{rank}", $"{nickName}", $"{Utils.ConvertBigNum(score)}점", rank, costumeId, petId, weaponId, magicBookId, gumgiIdx, guildName, maskIdx,hornIdx,suhoAnimal);
                    }
                    else
                    {
